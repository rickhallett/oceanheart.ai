        ...
        for i, pdf_path in enumerate(pdf_files, 1):
            try:
                ...
                if i % self.batch_size == 0 and i < len(pdf_files):
                    logger.info(f"Pausing for {self.delay_between_batches} seconds after sending {self.batch_size} emails")
...
def parse_arguments():
    ...
    parser.add_argument("--sender-email", required=True, help="Email address to send from")
    ...
    parser.add_argument("--username", help="SMTP username (default: sender email)")
    parser.add_argument("--password", help="SMTP password")
    parser.add_argument("--no-tls", action="store_true", help="Disable TLS")
    parser.add_argument("--subject-prefix", default="PDF Document: ", help="Prefix for email subject lines")
    parser.add_argument("--batch-size", type=int, default=5, help="Number of emails to send before pausing")
    parser.add_argument("--delay", type=int, default=30, help="Seconds to wait between batches")
